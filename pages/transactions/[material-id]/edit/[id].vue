...
const route = useRoute('/transactions/:material-id/edit/:id')

const { data: transactionsData, suspense } = useQuery({
  queryKey: ['transactions', route.params.id, route.params.materialid],
  queryFn: () => $fetch(`/api/transactions/item/${route.params.id}`),
...
const { handleSubmit } = useForm({
  ...
  initialValues: {
    year: transactionsData.value?.year.toString(),
    month: transactionsData.value?.month.toString(),
    startingBalance: transactionsData.value?.startingBalance,
    endingBalance: transactionsData.value?.endingBalance,
    receiptsAccountable: transactionsData.value?.receiptsAccountable,
    receiptsWarehouse: transactionsData.value?.receiptsWarehouse,
    issuesProduction: transactionsData.value?.issuesProduction,
  },
})
...
const { mutate: createMaterialMutation } = useMutation<Material>({
  mutationFn: data => $fetch(`/api/transactions/${route.params.id}`, {
    method: 'PATCH',
